...
import numpy as np
from utils import compute_norm, filter_none
...
class DynamicBaseModel(Module):
    ...
    # 这是评估模型性能的主函数，接收一系列时间戳，并基于指定的数据集分割（训练、验证或测试）执行评估。
    ...
    def calc_metrics(self, per_graph_ent_embeds, g_list, t_list, hist_embeddings, start_time_tensor, cur_t, split='valid'):
        ...
        for g, t, ent_embed in zip(g_list, t_list, per_graph_ent_embeds):
            if split == 'train':
                ...
                if split == 'valid':
                    loss_tail = self.valid_link_prediction(ent_embed, self.rel_embeds, triplets, neg_tail_samples, labels, all_embeds_g,
                                                           corrupt_tail=True)
                    loss_head = self.valid_link_prediction(ent_embed, self.rel_embeds, triplets, neg_head_samples, labels, all_embeds_g,
                                                           corrupt_tail=False)
                    total_loss += loss_tail + loss_head
                ranks.append(rank)
                # total_loss += loss_tail + loss_head
    ...
    # 基于生成的实体嵌入和图结构，计算并返回模型的评估指标和总损失。

    def valid_link_prediction(self, ent_embed, rel_embeds, triplets, neg_samples, labels, all_embeds_g, corrupt_tail=True):
        # pprint(triplets)
    ...
    def calc_metrics_single_graph(self, ent_mean, rel_enc_means, all_ent_embeds, samples, graph, time, eval_bz=100):
        ...
        return ranks
    # 为单个图计算链接预测的性能指标，具体来说是计算替换头实体（subject）和尾实体（object）时的排名。
    ...
    # 扰动三元组中的一个元素（头实体或尾实体），并计算每次扰动后目标实体的排名。
    ...
    def train_link_prediction(self, ent_embed, triplets, neg_samples, labels, all_embeds_g, corrupt_tail=True):
        ...
        predict_loss = torch.where(torch.isnan(predict_loss), torch.full_like(predict_loss, 0), predict_loss)
        return predict_loss
    # 计算链接预测任务的损失，用于训练阶段。它处理正样本三元组和负样本，并使用交叉熵损失函数计算预测损失。
    ...
    # 根据指定的得分函数（如distmult或complex）和模式（针对尾实体或头实体的预测）计算三元组的得分。
    ...
    def get_all_embeds_Gt(self, convoluted_embeds, g, t, prev_graph_embeds_list,
        ...
        return all_embeds_g
    # 为给定的图和时间点生成所有实体的嵌入。这个函数处理实体嵌入的更新，考虑到了时间变化和图结构的动态性。
    ...
    # 基于给定的时间列表（t_list）、序列长度（seq_len）和图字典（graph_dict），
    # 生成图批处理列表和时间批处理列表。
    ...
    # 为模型的前向传播过程准备历史嵌入、开始时间张量、历史嵌入的Transformer表示和注意力掩码。
    ...
    # 类似于get_per_graph_ent_embeds，但专注于生成每个图的实体嵌入的Transformer表示。
    ...
    # 更新历史嵌入的Transformer表示和注意力掩码，以反映每个时间点图的动态变化。
    ...
    def get_prev_embeddings(self, g_batched_list_t, history_embeddings, start_time_tensor, cur_t,
        ...
        #     time_diff_tensor)
    # 从历史嵌入中提取前一时间点的图嵌入，并计算时间差张量和局部注意力掩码。
    ...
    # 分别为训练/验证集和所有数据集（包括测试集）计算真实的头部和尾部实体信息。
    ...
    # 实现负采样逻辑，生成替换头实体或尾实体的负样本三元组及其对应的标签。
